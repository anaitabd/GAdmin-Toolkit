...
export default function CredentialForm({ initialData, onSubmit, onCancel }: CredentialFormProps) {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    try {
      ...
      setJsonError('')
      onSubmit({ name, domain: domain.trim() || null, cred_json: parsed, active })
    } catch {
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <input type="text" value={domain} onChange={e => setDomain(e.target.value)}
          ...
          className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500" />
        <p className="text-xs text-gray-500 mt-1">The email domain this credential controls (used to match sender users).</p>
      </div>
