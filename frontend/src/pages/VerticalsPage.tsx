...
function VerticalForm({ initial, onSubmit, onCancel, isPending }: {
    ...
    <div className="space-y-4">
      ...
      <div className="flex justify-end gap-2 pt-4">
        ...
        <button type="button" onClick={() => onSubmit({
          name, description: description || undefined, status, notes: notes || undefined
        })} disabled={!canSubmit}
...
export default function VerticalsPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['verticals', { search: search || undefined, status: statusFilter || undefined, limit, offset }],
    queryFn: () => verticalsApi.getAll({ search: search || undefined, status: statusFilter || undefined, limit, offset })
  })
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: any }) => verticalsApi.update(id, data),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['verticals'] })
  })

  const deleteMutation = useMutation({
    mutationFn: verticalsApi.deleteById,
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['verticals'] })
  })

  const [modalOpen, setModalOpen] = useState(false)
  const [editingItem, setEditingItem] = useState<Vertical | null>(null)
  const [deleteItem, setDeleteItem] = useState<Vertical | null>(null)
  const [mutationError, setMutationError] = useState('')

  const openCreate = () => { setEditingItem(null); setModalOpen(true) }
  const openEdit = (item: Vertical) => { setEditingItem(item); setModalOpen(true) }
  ...
  return (
    <div>
      ...
      <Modal isOpen={modalOpen} onClose={closeModal} title={editingItem ? 'Edit Vertical' : 'Add Vertical'}>
        <VerticalForm
          initial={editingItem}
