...
import type { Creative, Offer } from '../api/types'
...
function CreativeForm({ initial, onSubmit, onCancel, isPending }: {
  ...
  const canSubmit = offerId && subject.trim() && fromName.trim() && htmlContent.trim() && !isPending
    ...
    <div className="space-y-4">
      ...
      <div className="flex justify-end gap-2 pt-4">
        ...
        <button type="button" onClick={() => onSubmit({
          offer_id: parseInt(offerId), subject, from_name: fromName, html_content: htmlContent, status
        })} disabled={!canSubmit}
...
export default function CreativesPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['creatives', { search: search || undefined, offer_id: offerFilter || undefined, limit, offset }],
    queryFn: () => creativesApi.getAll({ search: search || undefined, offer_id: offerFilter ? parseInt(offerFilter) : undefined, limit, offset })
  })
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: any }) => creativesApi.update(id, data),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['creatives'] })
  })

  const deleteMutation = useMutation({
    mutationFn: creativesApi.deleteById,
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['creatives'] })
  })

  const [modalOpen, setModalOpen] = useState(false)
  const [editingItem, setEditingItem] = useState<Creative | null>(null)
  const [deleteItem, setDeleteItem] = useState<Creative | null>(null)
  const [mutationError, setMutationError] = useState('')

  const openCreate = () => { setEditingItem(null); setModalOpen(true) }
  const openEdit = (item: Creative) => { setEditingItem(item); setModalOpen(true) }
  ...
  return (
    <div>
      ...
      <Modal isOpen={modalOpen} onClose={closeModal} title={editingItem ? 'Edit Creative' : 'Add Creative'}>
        <CreativeForm
          initial={editingItem}
