...
export default function BackendLogsPage() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () => logsApi.getBackendLogs({ 
      level: levelFilter || undefined, 
      date_from: dateFrom || undefined, 
      date_to: dateTo || undefined, 
      search: search || undefined, 
                        ...
                        <tr key={`${log.id}-expanded`}>
                          <td colSpan={columns.length + 1} className="px-8 py-4 bg-gray-50">
                            <div className="text-sm space-y-3">
                              ...
                              {log.stack_trace && (
                                <div>
                                  <div className="font-medium text-gray-700 mb-2">Stack Trace:</div>
                                  <pre className="bg-white p-3 rounded border border-gray-200 overflow-x-auto text-xs text-red-600">
                                    {log.stack_trace}
                                  </pre>
                                </div>
                              )}
