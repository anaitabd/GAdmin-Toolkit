...
import type { Subject, Offer } from '../api/types'
...
function SubjectForm({ initial, onSubmit, onCancel, isPending }: {
...
export default function SubjectsPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['subjects', { search: search || undefined, offer_id: offerFilter || undefined, limit, offset }],
    queryFn: () => subjectsApi.getAll({ search: search || undefined, offer_id: offerFilter ? parseInt(offerFilter) : undefined, limit, offset })
  })
  ...
  const deleteMutation = useMutation({
    mutationFn: subjectsApi.deleteById,
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['subjects'] })
  })
  ...
  const openEdit = (item: Subject) => { setEditingItem(item); setModalOpen(true) }
  ...
  return (
    <div>
      ...
      <Modal isOpen={modalOpen} onClose={closeModal} title={editingItem ? 'Edit Subject' : 'Add Subject'}>
        <SubjectForm
          initial={editingItem}
