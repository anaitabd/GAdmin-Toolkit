...
import type { AffiliateNetwork } from '../api/types'
...
const columns: Column<AffiliateNetwork>[] = [
  { key: 'id', header: 'ID' },
...
function AffiliateNetworkForm({ initial, onSubmit, onCancel, isPending }: {
  initial?: AffiliateNetwork | null
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <input type="url" value={apiEndpoint} onChange={e => setApiEndpoint(e.target.value)}
          placeholder="https://api.network.com"
          className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:ring-indigo-500" />
      </div>
      ...
      <div className="flex justify-end gap-2 pt-4">
        ...
        <button type="button" onClick={() => onSubmit({
          name, description: description || undefined, contact_name: contactName || undefined,
          contact_email: contactEmail || undefined, api_endpoint: apiEndpoint || undefined,
...
export default function AffiliateNetworksPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['affiliate-networks', { search: search || undefined, status: statusFilter || undefined, limit, offset }],
    queryFn: () => affiliateNetworksApi.getAll({ search: search || undefined, status: statusFilter || undefined, limit, offset })
  })

  const createMutation = useMutation({
    mutationFn: affiliateNetworksApi.create,
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['affiliate-networks'] })
  })

  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: any }) => affiliateNetworksApi.update(id, data),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['affiliate-networks'] })
  })

  const deleteMutation = useMutation({
    mutationFn: affiliateNetworksApi.deleteById,
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['affiliate-networks'] })
  })

  const [modalOpen, setModalOpen] = useState(false)
  const [editingItem, setEditingItem] = useState<AffiliateNetwork | null>(null)
  const [deleteItem, setDeleteItem] = useState<AffiliateNetwork | null>(null)
  const [mutationError, setMutationError] = useState('')

  const openCreate = () => { setEditingItem(null); setModalOpen(true) }
  const openEdit = (item: AffiliateNetwork) => { setEditingItem(item); setModalOpen(true) }
  ...
  return (
    <div>
      ...
      <Modal isOpen={modalOpen} onClose={closeModal} title={editingItem ? 'Edit Affiliate Network' : 'Add Affiliate Network'}>
